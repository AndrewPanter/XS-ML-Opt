# %%
# Build and train the ResNet model, including the hyperparameter process
...
def EvolutionarySearch(best_grid_config):
    population_size = 20  # Define a population size for evolution
...
callback = ModelCheckpoint('checkpoints/resnet_best_model.keras', 
                                      monitor='val_mae', 
                                      save_best_only=True)
history = final_model.fit(X_train, ytrain, epochs=300, batch_size=batch_size, validation_split=0.2, callbacks=[callback])
