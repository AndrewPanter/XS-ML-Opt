# %%
# Build and train the CNN model, including the hyperparameter process
...
def build_CNN(layers, nodes, dropout_rate, learning_rate, input_shape=(399, 1)):
    ...
    model.compile(loss='mse', optimizer=Adam(learning_rate), metrics=['mae', 'R2Score', 'MeanAbsolutePercentageError'])
    return model

# make training and evaluating model function for hyperparameter tuning
def train_evaluate(model, batch_size, epochs, X_train, ytrain, X_val, yval):
    ...
    loss, mae, r2, mape = model.evaluate(X_test, ytest)
    return mae, r2, history, mape

# make list of all hyperparameter combos tried and output metrics
metrics_list = []
...
# cycle through all layer and node combos
for layer in layers:
    for nodes in nodes_per_layer:
        ...
        mae, r2, manual_history, mape = train_evaluate(model, batch_size, epochs, X_train, ytrain, X_test, ytest)
        ...
        metrics_list.append(["Baseline", layer, nodes, batch_size, learning_rate, dropout_rate, epochs, mae, r2, mape])
...
# random search
print("random search")
def RandomSearch(best_baseline, num_trials=10): 
    ...
    for _ in range(num_trials):
        ...
        mae, r2, random_hisotry, mape = train_evaluate(model, bs, epochs, X_train, ytrain, X_test, ytest)
        if mae < best_random_mae:
            best_random_mae = mae
            best_random_r2 = r2
            best_random_config = (layers, nodes, dr, bs, lr, epochs)

        metrics_list.append(["Random", layers, nodes, bs, lr, dr, epochs, mae, r2, mape])
...
def GridSearch(best_random_config):
    ...
    # refine values by adding and subtracting by small increments
    fine_learning_rates = np.linspace(learning_rate - 0.0005, learning_rate + 0.0005, num=5)
    fine_dropout_rates = np.linspace(dropout_rate - 0.05, dropout_rate + 0.05, num=5)
    fine_batch_sizes = [batch_size//2, batch_size, batch_size*2]

    # test all combos and find lowest MAE
    for lr in fine_learning_rates:
        for dr in fine_dropout_rates:
            for bs in fine_batch_sizes:
                print("Current Model:", layers, nodes, dr, bs, lr, epochs)
                model = build_CNN(layers, nodes, dr, lr)
                mae, r2, grid_history, mape = train_evaluate(model, bs, epochs, X_train, ytrain, X_test, ytest)
                if mae < best_grid_mae:
                    best_grid_mae = mae
                    best_grid_r2 = r2
                    best_grid_config = (layers, nodes, dr, bs, lr, epochs)
                metrics_list.append(["Grid", layers, nodes, bs, lr, dr, epochs, mae, r2, mape])
...
def EvolutionarySearch(best_grid_config):
    ...
    for generation in range(0, generations):
        ...
        for individual in population:
            ...
            mae, r2, hisotry, mape = train_evaluate(model, individual['batch_size'], individual['epochs'], X_train, ytrain, X_test, ytest)
            individual['fitness'] = mae
            individual['r2'] = r2

            metrics_list.append(["Evolution", individual['layers'], individual['nodes'], individual['batch_size'], individual['learning_rate'], individual['dropout_rate'], individual['epochs'], mae, r2, mape])
...
# %%
# put metrics from all parameter combos into dataframe and save file
metrics_df = pd.DataFrame(metrics_list, columns =['Hyperparameter Strategy', 'Layers', 'nodes_per_layer', 'batch_size', 'learning_rate', 'dropout_rate', 'epochs', 'MAE', 'R2', 'MAPE'])

metrics_df = metrics_df.sort_values(by='MAE', ascending=True)

metrics_df.to_csv('metrics_CNN-test.txt', sep='\t', index=False)

# pull out best parameter combo
...
final_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate), loss='mse', metrics=['mae', 'R2Score', 'MeanAbsolutePercentageError'])
...
final_loss, final_mae, final_r2, final_MAPE = final_model.evaluate(X_test, ytest)
y_pred_test = final_model.predict(X_test)
y_pred_train = final_model.predict(X_train)

# print results
print("Test loss: ", final_loss, "Test MAE: ", final_mae, "Test R2:", final_r2, "Test MAPE:",final_MAPE)
...
plt.ylabel('MAE')
plt.legend()
plt.savefig('MAE_plot_CNN-test.png')
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('loss_plot_CNN-test.png')

plt.figure()
plt.scatter(y_pred_train, ytrain, label = 'Training Data')
plt.scatter(y_pred_test, ytest, label = 'Testing Data')
plt.xlabel('Predicted Outcome')
plt.ylabel('Actual Outcome')
plt.legend()
plt.show()
plt.savefig('diagonal_validation_CNN-test.png')
